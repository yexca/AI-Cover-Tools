...
然后在浏览器中打开 `http://127.0.0.1:7657`。启动脚本始终使用项目内的 `env`；如果尚未安装环境，会显示明确的错误提示。也可以继续传递 Uvicorn 参数，例如 `run-webui.bat --port 8000`。

## WebUI MVP

WebUI 是一个 ComfyUI 风格的音频工作流编辑器，包含单音频与文件夹输入、基于模型的分离、音频切片、峰值标准化和文件夹输出。左侧模型列表主要按功能分类，并可按架构筛选；模型节点的输出端口由模型注册表元数据生成。输出文件夹支持标准命名和智能分类；智能分类可接收多个由模型分离得到的输入，并按 `<模型>_<类型>/<原始相对文件夹>/<歌曲>.<扩展名>` 保存。

可以把左侧项目拖入画布，或双击添加节点。拖动节点上任意非交互区域即可移动；选中节点后，其上方会显示用于复制和确认删除的简洁工具栏，右下角手柄用于调整大小。单音频控件使用浏览器的文件选择器，并将所选文件上传到本地服务；输入与输出文件夹仍使用 Windows 原生文件夹选择器。连接可以双向拖动，也可依次点击两个兼容端口。运行前，服务端会检查路径、上传文件、模型、端口和图结构；无法到达输出节点的分支不会执行，同名输出按照所选的自动编号、覆盖或跳过策略处理。

界面默认跟随浏览器语言，目前支持简体中文、日语和英语。顶部语言选择器可以覆盖浏览器设置，并在本机记住选择。

WebUI 启动时会立即显示已保存的注册表，同时在后台扫描本地已安装模型的变化。该扫描只读取本地文件名和元数据，不会下载模型，也不会把模型载入 GPU。使用**刷新模型**可以再次执行本地扫描；在线模型目录同步是单独的手动操作，避免网络缓慢或不可用时阻塞启动。无法可靠识别功能或输出的模型会标记为待确认，不会静默猜测。
