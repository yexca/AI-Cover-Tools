...
可以把左侧项目拖入画布，或双击添加节点。选中节点后可在右侧“属性”面板编辑参数；路径字段的**浏览**按钮会在运行 WebUI 的电脑上打开 Windows 原生文件或文件夹选择器。连接既可以从输出端口拖到输入端口，也可以反向拖动；依次点击两个兼容端口仍作为备用操作保留。运行前，服务端会检查必填路径、模型可用性、端口和图结构，并在画布上标出对应节点与连接。
