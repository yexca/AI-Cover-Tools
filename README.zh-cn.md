...
然后在浏览器中打开 `http://127.0.0.1:8188`。启动脚本始终使用项目内的 `env`；如果尚未安装环境，会显示明确的错误提示。也可以继续传递 Uvicorn 参数，例如 `run-webui.bat --port 8000`。

## WebUI MVP

WebUI 是一个早期的 ComfyUI 风格音频工作流编辑器。首版节点包括单文件输入、文件夹输入、基于模型的音频处理和文件夹输出。左侧模型列表主要按功能分类，并可按架构筛选；模型节点的输出端口由模型注册表元数据生成。

可以把左侧项目拖入画布，或双击添加节点。选中节点后可在右侧“属性”面板编辑参数，并可拖动节点右下角的手柄调整大小。单文件与输出节点会直接在画布上概览所选文件名或目标文件夹；红色的节点标题栏删除按钮会先弹出简洁的确认框，再执行删除。路径字段的**浏览**按钮会在运行 WebUI 的电脑上打开 Windows 原生文件或文件夹选择器。连接既可以从输出端口拖到输入端口，也可以反向拖动；依次点击两个兼容端口仍作为备用操作保留。运行前，服务端会检查必填路径、模型可用性、端口和图结构，并在画布上标出对应节点与连接。
