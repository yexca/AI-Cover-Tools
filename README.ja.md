...
左側の項目をキャンバスへドラッグするか、ダブルクリックしてノードを追加します。ノードを選択すると、右側の「プロパティ」パネルで設定を編集でき、右下のハンドルをドラッグしてサイズも変更できます。単一ファイル入力と出力ノードは、選択したファイル名または出力先フォルダーをキャンバス上に要約表示します。ヘッダーの赤い削除ボタンは、コンパクトな確認ポップオーバーを表示してからノードを削除します。パス欄の**参照**ボタンは、WebUI を実行している PC 上で Windows 標準のファイル／フォルダー選択画面を開きます。出力ポートから入力ポート、またはその逆方向へドラッグして接続できます。互換性のある二つのポートを順にクリックする操作も予備手段として利用できます。実行前には、必須パス、モデル、ポート、グラフ構造をサーバー側で確認し、該当するノードと接続をキャンバス上に表示します。
