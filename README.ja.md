...
その後、ブラウザーで `http://127.0.0.1:7657` を開きます。起動スクリプトは常にプロジェクト内の `env` を使用し、環境が未インストールの場合は明確なエラーを表示します。`run-webui.bat --port 8000` のように Uvicorn のオプションを渡すこともできます。

## WebUI MVP

WebUI は ComfyUI 風のオーディオワークフローエディターで、単一音声とフォルダー入力、モデル分離、音声スライス、ピークノーマライズ、フォルダー出力に対応します。左サイドバーではモデルを主に機能別に分類し、アーキテクチャーで絞り込めます。モデルノードの出力ポートはモデルレジストリーのメタデータから生成されます。出力フォルダーは標準命名とスマート分類に対応し、スマート分類ではモデル分離由来の複数入力を `<モデル>_<種類>/<元の相対フォルダー>/<曲>.<拡張子>` に保存します。

左側の項目をキャンバスへドラッグするか、ダブルクリックしてノードを追加します。ノードの非操作領域ならどこからでもドラッグして移動でき、選択すると上部に複製と確認付き削除のコンパクトなツールバーが表示されます。右下のハンドルではサイズを変更できます。単一音声はブラウザーのファイル選択を使ってローカルサービスへアップロードし、入力・出力フォルダーは Windows 標準のフォルダー選択を引き続き使用します。接続はどちらの方向からでもドラッグでき、互換ポートを順にクリックする方法も利用できます。実行前にパス、アップロード、モデル、ポート、グラフ構造を検証します。出力フォルダーへ到達しない分岐は実行されず、同名出力は選択した自動採番、上書き、またはスキップ方針で処理されます。
